...
interface IAccordionProps {
  ...
  homeGlobalAssets: IHomeGlobalAssets[];
}
...
const Accordion: React.FC<IAccordionProps> = ({ items, homeGlobalAssets }) => {
  ...
  const [arrowColors, setArrowColors] = useState<IHomeGlobalAssets[]>(() => {
    const classList = htmlClassNameRef.current;
    return homeGlobalAssets.filter((arrow) =>
  ...
  useEffect(() => {
    const filteredArrows = homeGlobalAssets.filter((arrow) =>
      themeColor === "light"
        ? arrow.alt.includes("Chevron Arrow Black")
        : arrow.alt.includes("Chevron Arrow White")
    );
