...
export async function POST(req: Request) {
  ...
  try {
    const formattedMitFeatures = Array.isArray(mit_features) ? mit_features.join(', ') : '';
    ...
    const formRes = await fetch(`https://api.hsforms.com/submissions/v3/integration/submit/${HUBSPOT_PORTAL_ID}/${HUBSPOT_FORM_ID}`, {
      ...
      body: JSON.stringify({
        fields: [
          { name: 'name_and_surname', value: name_and_surname },
          { name: 'museum_company', value: museum_company },
